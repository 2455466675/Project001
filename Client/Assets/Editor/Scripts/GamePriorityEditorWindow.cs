...
public class GamePriorityEditorWindow : EditorWindow
{
        ...
        foreach (Assembly assembly in assemblies)
        {
            try
            {
                ...
                foreach (Type type in types)
                {
                    ...
                    {
                        int priority = 0;
                        var existingData = classDataList.Find(d => d.type == type.FullName);
                        if (existingData != null)
                        {
                            priority = existingData.priority;
                        }
                        classDataList.Add(new ClassPriorityData() { type = type.FullName, priority = priority });
                    }
                }
            }
        ...
        if (File.Exists(fullPath))
        {
            try
            {
                ...
                if (wrapper != null && wrapper.items != null)
                {
                    ...
                    {
                        var existingData = classDataList.Find(d => d.type == data.type);
                        if (existingData != null)
                        {
    ...
    private void OnGUI()
    {
        ...
        else
        {
            ...
            {
                EditorGUILayout.BeginHorizontal();
                EditorGUILayout.LabelField(classDataList[i].type, GUILayout.Width(position.width - 150));
